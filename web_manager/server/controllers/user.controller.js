...
userCtrl.getNumberPages = async function (req, res) {
  var table =req.body.table;
  var tableSelect=mangerModelAdmin(table);
  var condition = tableSelect.getConditionManisfest(req.currentUser);
  var itemSelect= tableSelect.getValueToSelectToFind(req.body.dataFind); 
  var dataTableSQL= "SELECT COUNT(*) FROM " + tableSelect.getNameTable() +" WHERE " +condition.sql + itemSelect.sql;
  var allParams = [...condition.params, ...itemSelect.params];
  var infoData = await tableSelect.queryDatabaseDetail(dataTableSQL, allParams);
  return returnInfoQuery(res,infoData)
}
...
userCtrl.updateFistPages= async  function (req, res) {
  ...
  var result = await tableSelect.queryDatabaseDetail(sqlUpdate, [req.body['pages_content_id']]);
...
userCtrl.getInfoImageInfoId = async function (req, res) {
    ...
    sql= sql + " AND image_id= ? AND deleteflag = 0;";
    var x= await knex.raw(sql, [req.body.image_id]);
...
userCtrl.getProductByCompany  = async function(req, res) {
      var sql=  "SELECT product.* FROM product  WHERE company_id= ? AND deleteflag = 0 ;";
...
userCtrl.getProductImageByCompany  = async function(req, res) {
      var sql=  "SELECT product_image.* FROM product_image  WHERE product_id= ? AND deleteflag = 0 ;";
      var x= await knex.raw(sql, [req.body.product_id]);
      if ((x==null)||(x.length==0))  return returnNotFound(res,{ message: "acao Not exitting "},WarningInfo.DATA_NOT_EXSITING);
      return returnOK(res,x[0]);
}

userCtrl.getLastdayProductBuy  = async function(req, res) {
  var sql=  "select extract(YEAR from updated_at) as month,sum(Total) as total_value,COUNT(*) as total from product_buy  WHERE updated_at  >= now() - INTERVAL ? day AND deleteflag = 0 group by month;";
  var x= await knex.raw(sql, [req.body.time]);
  if ((x==null)||(x.length==0))  return returnNotFound(res,{ message: "acao Not exitting "},WarningInfo.DATA_NOT_EXSITING);
  return returnOK(res,x[0]);
}
...
userCtrl.setDataApprovedInfo  = async function(req, res) {
    ...
    if(req.body.companyname!=null){
        var editCom = squel.update().table("company");
          ...
          .set("updated_at","NOW()",{dontQuote: true})
          .set("deleteflag",0);
        var p = editCom.toParam();
        y = await knex.raw(p.text, p.values);
    }
    if ((x==null)||(x.length==0))  return returnNotFound(res,{ message: "Không tồn tại dữ liệu"},WarningInfo.DATA_NOT_EXSITING);
...
userCtrl.getTagName = async  function(req, res) {
  ...
  var sanitizedTable = req.body.table.replace(/[^a-zA-Z0-9_]/g, '');
  var sanitizedShort = req.body.short.replace(/[^a-zA-Z0-9_]/g, '');
  var authen = squel.select().from(sanitizedTable)
                    .where(sanitizedShort+ "= ?", data)
                    .where("deleteflag=0");
  var p = authen.toParam();
  var result= await knex.raw(p.text, p.values);
  if ((result==null)||(result.length==0)) {
